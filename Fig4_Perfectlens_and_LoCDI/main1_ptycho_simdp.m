...
%   ss(m.cell_suppBb==1)=max(ss(:));  ss(m.cell_suppB==1)=0;
dd = (m.h2o + m.cell).*m.cell_suppB;
m.supp = m.cell_supp | m.lacey_supp;     

%% difference flux
for run_id = [2,4,7,9]  %  1:5 no lacey 6:10 with lacey
...
%         centery(scanid) = sz/2-(step*scany-sy) +rand2;%
        ...
        if scanid==1;
        fprintf('dose = %.2s, flux in dp = %.2s, total flux for each scan = %.2s, total dose = %.2s, dose per um2 = %.2s\n ',...
            p.e.dose,p.e.integrated_counts,p.e.Io_cell, p.e.Io_cell2*scannumx.^2,p.e.Io_cell2*scannumx.^2./(p.e.scanarea.^2));
        end
